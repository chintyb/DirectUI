...
void CBlueRenderEngineUI::DoPaintPrettyText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, UITYPE_COLOR iTextColor, UITYPE_COLOR iBackColor, RECT* prcLinks, int& nLinkRects, UINT uStyle)
{
   ...
   while( *pstrText != _T('\0') ) 
   {
               ...
               && (pstrText[2] == _T(' ') || pstrText[2] == _T('>')) )
      {
         ...
         case _T('i'):  // Icon
            {
               ...
               if( isdigit(*pstrText) ) {
                  ...
                  if( *pstrText == _T(' ')) pstrText++;
                  CStdString sRes;
                  while( _istalnum(*pstrText) || *pstrText == _T('.') || *pstrText == _T('_')) sRes += *pstrText++;
         ...
         while( *pstrText != _T('\0') && *pstrText != _T('>')) pstrText++;
         pstrText++;
      }
      else if( *pstrText == _T('<') && pstrText[1] == _T('/'))
      {
         pstrText += 2;
         switch( *pstrText++ )
         {
