   ...
   for( ; ; ) 
   {
      ...
      *pstrText++ = _T('\0');
      ...
      if( *pstrText == _T('!') || *pstrText == _T('?') ) {
         TCHAR chEnd = *pstrText == _T('!') ? _T('-') : _T('?');
         while( *pstrText != _T('\0') && !(*pstrText == chEnd && *(pstrText + 1) == _T('>')) ) pstrText = ::CharNext(pstrText);
...
{
   while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || _istalnum(*pstr)) ) pstr++;
}
   ...
   *pstrText++ = _T('\0');
   ...
   while( *pstrText != _T('\0') && *pstrText != _T('>') && *pstrText != _T('/')) {
      ...
      *pstrText++ = _T('\0');
      ...
      if( chQuote != _T('\"') && chQuote != _T('\'')) return _Failed(_T("Expected attribute value"), pstrText);
      LPTSTR pstrDest = pstrText;
